...
`Context::sort` and `Context::merge` are the two non-Job primitives — they live directly in [context.inl](include/multi/details/context.inl) (with helpers in the `details` namespace) because they orchestrate multiple `range` dispatches in sequence rather than mapping to a single dispatch shape. Both are documented inline; the high-level structure:
- **`Context::merge`** — co-rank binary search picks K = workerCount·4 balanced split points across the two inputs in O(K·log(min(m,n))), then a single `range` dispatch runs serial `std::merge` per chunk in parallel. Below `MERGE_PARALLEL_THRESHOLD` (8192 total items) or with `workerCount < 2`, delegates straight to `std::merge`. Stability convention: equivalent elements from A precede those from B (matches `std::merge`); enforced inside `details::coRank` by the asymmetric `comp(b[j], a[i-1])` vs `!comp(b[j-1], a[i])` predicates.
- **`Context::sort`** — three-band dispatch keyed off `n`:
...
Both helpers are templated on `CtxT` (always `Context` here) so they can call back into `ctx->parallel`/`ctx->range`/`ctx->merge` without naming the global context — keeps them usable on any Context instance, including test-local pools. `chunkedSortImpl` has a forward declaration in the first `details` block (before `Context::sort`) because its definition lives in the second `details` block (after `Context::sort`, since it calls `ctx->merge` which needs Context to be complete).
