...
- `taskCount() == 2`: submit task 0 to a worker via `submit(Task)`, run task 1 inline on the caller, then spin on `remaining()` for task 0. The `parallel(a, b)` fast path — saves one push + `fencedNotify` + steal-loop iteration vs the generic submitBatch path. Measured `~−40%` on the dispatch-heavy `parallel_pair / 1k rounds` bench; neutral on recursive workloads with non-trivial per-leaf work.
